...
//fadepolicy
    ...
    function swipeInContent() {
        ...
        const justify  = document.querySelector('.justifyannouncement');
        const poster  = document.querySelector('.poster');
        const khukan  = document.querySelector('.khukanmove');
        ...
        if (justify) {
            let rect = justify.getBoundingClientRect();
            let windowHeight = window.innerHeight || document.documentElement.clientHeight;
            if (rect.top <= windowHeight && rect.bottom >= 0) {
                justify.classList.add('swipe-in');
                window.removeEventListener('scroll', swipeInContent);
            }
        }
        if (poster) {
            let rect = poster.getBoundingClientRect();
            let windowHeight = window.innerHeight || document.documentElement.clientHeight;
            if (rect.top <= windowHeight && rect.bottom >= 0) {
                poster.classList.add('swipe-in');
                window.removeEventListener('scroll', swipeInContent);
            }
        }
        if (khukan) {
            let rect = khukan.getBoundingClientRect();
            let windowHeight = window.innerHeight || document.documentElement.clientHeight;
            if (rect.top <= windowHeight && rect.bottom >= 0) {
                khukan.classList.add('swipe-in');
                window.removeEventListener('scroll', swipeInContent);
            }
        }
    }
